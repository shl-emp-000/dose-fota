...
import com.innovationzed.fotalibrary.DataModelClasses.OTAFlashRowModel_v0;
...
public class OTAFUHandler_v0 extends OTAFUHandlerBase implements FileReadStatusUpdater {
    ...
    public OTAFUHandler_v0(Context context, BluetoothGattCharacteristic otaCharacteristic, byte activeApp, long securityKey, String filepath, OTAFUHandlerCallback callback) {
        super(context, otaCharacteristic, activeApp, securityKey, filepath, callback);
...
//            mProgressText.setText("getContext().getResources().getText(R.string.ota_file_read_complete)");
            Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.ENTER_BOOTLOADER);
...
//                    showErrorDialogMessage("getContext().getResources().getString(R.string.alert_message_silicon_id_mismatch_error)", true);
            ...
            int programRowNum = Utils.getIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_NO);
            if (programRowNum == 0) {
                ...
                if (statusReceived.equalsIgnoreCase("00")) {
                    ...
                    if (fileChecksumByte.equalsIgnoreCase(checksumReceived)) {
                        ...
                        if (programRowNum < mFlashRowList.size()) {
                            Utils.setIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_NO, programRowNum);
                            Utils.setIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_START_POS, 0);
                            writeProgrammableData(programRowNum);
                        }
                        if (programRowNum == mFlashRowList.size()) {
...
//            mProgressText.setText("getContext().getResources().getText(R.string.ota_end_success)");

            setFileUpgradeStarted(false);
            storeAndReturnDeviceAddress();
            BluetoothLeService.disconnect();
            BluetoothLeService.unpairDevice(device);
            Toast.makeText(getContext(), "getResources().getString(R.string.alert_message_bluetooth_disconnect)", Toast.LENGTH_SHORT).show();
//            Intent finishIntent = getContext().getIntent();
//            getContext().finish();
...
//            showErrorDialogMessage("getContext().getResources().getString(R.string.alert_message_ota_error) + errorMessage", false);
    ...
    private void sendGetAppStatusCmd() {
        ...
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.GET_APP_STATUS);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_get_app_status)");
    }
    ...
    private void sendGetFlashSizeCmd() {
        ...
        Utils.setIntSharedPreference(getContext(), Constants.PREF_ARRAY_ID, Byte.valueOf(data[0]));
        mOtaFirmwareWrite.OTAGetFlashSizeCmd(mCheckSumType, data);
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.GET_FLASH_SIZE);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_get_flash_size)");
    }
    ...
    private void sendVerifyRowCmd() {
        ...
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.VERIFY_ROW);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_verify_row)");
    }

    private void sendVerifyChecksumCmd() {
        Utils.setIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_NO, 0);
        Utils.setIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_START_POS, 0);
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.VERIFY_CHECK_SUM);
        mOtaFirmwareWrite.OTAVerifyCheckSumCmd(mCheckSumType);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_verify_checksum)");
    }

    private void sendSetActiveAppCmd() {
        mOtaFirmwareWrite.OTASetActiveAppCmd(mCheckSumType, mActiveApp);
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.SET_ACTIVE_APP);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_set_active_application)");
    }

    private void sendExitBootloaderCmd() {
        mOtaFirmwareWrite.OTAExitBootloaderCmd(mCheckSumType);
        Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.EXIT_BOOTLOADER);
//        mProgressText.setText("getContext().getResources().getText(R.string.ota_end_bootloader)");
    }
    ...
    private void writeProgrammableData(int rowPosition) {
        ...
        if (modelData.mArrayId != Utils.getIntSharedPreference(getContext(), Constants.PREF_ARRAY_ID)) {
            /**
             * Writing the get flash command again to get the new row numbers
             * Changing the shared preference value
             */
            Utils.setIntSharedPreference(getContext(), Constants.PREF_ARRAY_ID, modelData.mArrayId);
            byte[] data = new byte[1];
            data[0] = (byte) modelData.mArrayId;
            mOtaFirmwareWrite.OTAGetFlashSizeCmd(mCheckSumType, data);
            Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.GET_FLASH_SIZE);
//            mProgressText.setText("getContext().getResources().getText(R.string.ota_get_flash_size)");
            ...
            if (rowNum >= mStartRow && rowNum <= mEndRow) {
                ...
                if (checkProgramRowCommandToSend(verifyDataLength)) {
                    ...
                    Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.PROGRAM_ROW);
                    Utils.setIntSharedPreference(getContext(), Constants.PREF_PROGRAM_ROW_START_POS, 0);
//                    mProgressText.setText("getContext().getResources().getText(R.string.ota_program_row)");
                    ...
                    Utils.setStringSharedPreference(getContext(), Constants.PREF_BOOTLOADER_STATE, "" + BootLoaderCommands_v0.SEND_DATA);
